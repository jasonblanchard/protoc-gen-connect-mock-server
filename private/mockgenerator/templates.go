...
const mockServerMethodTpl = `func ({{ .MockServerName }}) {{ .GoName }}(context.Context, *connect_go.Request[v1.{{ .InputName }}]) (*connect_go.Response[v1.{{ .OutputName }}], error) {
	resp := &connect_go.Response[v1.{{ .OutputName }}]{}
	resp.Msg = NewMock{{ .OutputName }}()
	return resp, nil
}`
...
const mockMessageTpl = `func NewMock{{ .Name }}() *v1.{{ .Name }}  {
	mock := &v1.{{ .Name }} { {{ range $field := .Fields }}
